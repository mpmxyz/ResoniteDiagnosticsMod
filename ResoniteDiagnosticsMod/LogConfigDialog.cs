...
internal class LogConfigDialog {
	public LogConfigDialog(World w) {
		...
		var textField = uiBuilder.TextField(ResoniteDiagnosticsMod.CurrentDefinitions, parseRTF: false);
		...
		uiBuilder.Button("Update", RadiantUI_Constants.Sub.GREEN).LocalPressed += (b, e) => {
			...
			if (!ResoniteDiagnosticsMod.SetDefinitions(ref newConfig, b.World.Types)) {
